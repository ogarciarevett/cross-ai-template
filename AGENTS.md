...
- The per-tool mirrors (`.claude/`, `.gemini/`, `.opencode/`, `.cursor/`, `.ai/generated/`) are
  **gitignored / local-only** — the repo carries NO duplicated skill/agent/command copies, only
  the single `.ai/` source. Regenerate the mirrors with `sh scripts/install.sh` (or the sync
  script) once per clone, or your tools see zero skills. The ONLY generated files committed are the
  root contract entries `AGENTS.md` / `CLAUDE.md` / `GEMINI.md` — kept so the repo stays readable
  on GitHub and the contract loads without a build step; the pre-commit hook gates only those.
