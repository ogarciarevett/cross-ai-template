...
metadata:
  ...
  vendored: "Design skill only — adapted to be dependency-free. impeccable's Node CLI (detector, palette/context helpers, live-server) is NOT bundled; those steps degrade to manual fallbacks. live mode is unavailable here (see reference/live.md). Builds on Anthropic's frontend-design skill; Apache-2.0."
---

Designs and iterates production-grade frontend interfaces. Real working code, committed design choices, exceptional craft.

> **Vendored, dependency-free copy.** This is impeccable's design skill adapted for a
> language-agnostic template: pure craft knowledge, no Node runtime. Wherever a step below
> mentions impeccable's CLI (`npx impeccable`) it's an **optional accelerant** — when it's
> not installed (the default here), follow the manual fallback. To get the full toolchain
> (bundled anti-pattern detector, palette seed, live browser mode), install it directly:
> `npx impeccable skills install`. Upstream: https://github.com/pbakaus/impeccable

## Setup

Do these steps before proceeding:

1. **Load project context.** If the project has a `PRODUCT.md` (strategic: users, brand, principles) and/or `DESIGN.md` (visual: colors, typography, components), read them — every command builds on them. If neither exists and this is real design work, start with `reference/init.md` to establish that context first. (impeccable's CLI automates this via `context.mjs`; without it, just read the files directly.)
...
5. **If the project is brand-new (no existing CSS tokens / theme / committed brand colors found in step 3)**, choose a brand anchor color deliberately (see the **Color strategy** guidance below) and compose the rest of the palette (bg, surface, ink, accent, muted) around it. Use OKLCH throughout. **Skip this step if step 3 found committed brand colors in existing tokens; in that case identity-preservation wins.** (impeccable's CLI can suggest a brand seed via `palette.mjs`; without it, pick the anchor yourself using the color rules.)

## Design guidance

Produce ready-to-ship, production-grade code, not prototypes or starting points. Take no shortcuts unless the user asks for them (when in doubt, ask). Don't stop until arriving at a complete implementation (beautiful, responsive, fast, precise, bug-free, on brand). You take attention to detail seriously: every page, section or component crafted is battle tested using the tools available to you (browser screenshotting, computer use, etc). Your model is capable of extraordinary work. Don't hold back.
...
> **Note — common model tell (typography ceilings often missed):**
> - Hero clamp() max ≤ 6rem. 8–11rem (128–176px) reads as comically loud, not bold. <!-- rule:skill-typo-codex-hero-ceiling-repeat -->
> - Display letter-spacing ≥ -0.04em. A default of -0.05 to -0.085em on display H1s makes the letters touch and reads as cramped. -0.02 to -0.03em is plenty for tight grotesque display; -0.04em is the floor. <!-- rule:skill-typo-codex-tracking-repeat -->
...
> **Note — common model tell (image hover):** Never animate `<img>` elements on hover. This includes any `transform` on `:hover` of an image, AND `.group:hover .group-hover\:scale` / `.group:hover .group-hover\:rotate` / `.group:hover .group-hover\:translate` patterns from Tailwind that animate a child image via a parent hover. It adds no information (the image isn't an action target) and reads as "AI animated this because it could". If a card needs hover feedback, animate the card's background, border, or shadow. Never the image, never via the image's parent. <!-- rule:skill-interaction-gemini-no-image-hover -->
...
> **Note — common model tells (refuse-and-rewrite):**
...
> - **`border-radius: 32px+` on cards / sections / inputs.** Over-rounding. Cards top out at 12–16px; full-pill is fine for tags/buttons. Picking 24/28/32/40px on a card is a tell; no brand wants "insanely rounded". <!-- rule:skill-ban-codex-over-round -->
...
| `live` | Iterate | Visual variant mode (requires impeccable's optional CLI — see [reference/live.md](reference/live.md)) | [reference/live.md](reference/live.md) |

### Routing rules

1. **No argument**: the user is asking "what should I do?" Make the menu context-aware instead of static. If there's no `PRODUCT.md` yet, you're effectively in `init` — finish that first. Otherwise lead with the **2-3 highest-value next commands**, each with a one-line reason, then show the full menu (the table above, grouped by category). **Never auto-run a command; the recommendation is a suggestion the user confirms.** Reason from what you can see: no `DESIGN.md` but code exists → `document`; never critiqued → `critique <surface>`; a low prior critique score or open P0/P1 → `polish`; one surface in the dirty git tree → scope `audit`/`polish` to those files. (impeccable's CLI can compute these signals and run its bundled detector over changed files; without it, infer them from the repo state yourself.)
...
Setup (context gathering, register) is already loaded by then; sub-commands don't re-invoke the parent skill.
...
> **Pinning** (standalone `/<command>` shortcuts that invoke `/impeccable <command>` directly) is a feature of impeccable's CLI and is not available in this dependency-free copy. Install the upstream CLI if you want it.
