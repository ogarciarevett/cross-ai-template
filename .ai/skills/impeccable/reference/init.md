...
2. Check the project's Content-Security-Policy. Live mode (an optional feature, see [live.md](live.md)) needs `http://localhost:8400` allowed in `script-src` and `connect-src`. If you find a CSP defined in middleware, meta tags, or headers, surface the file(s) and ask the user to add that origin; otherwise there's nothing to do. (impeccable's CLI — `npx impeccable`, not bundled with this dependency-free skill — automates this detection and patching.)
