...
2. **Derive a slug** for this target — a short, stable, filesystem-safe id from the resolved path or URL (e.g. `site/pages/index.astro` → `site-pages-index`). You'll use it to name the saved critique under `.impeccable/critique/`. If the target is vague or root-level, treat the slug as null: skip persistence and trend for this run, but continue the critique.
...
Deterministic scan (optional accelerant): impeccable ships a bundled anti-pattern detector run via its CLI (`npx impeccable`, not bundled with this dependency-free skill). When it's available, run it over the markup target for a no-network, rule-based pass and reuse its JSON findings:

```bash
npx impeccable detect --json [target]
```

When the CLI is **not** available (the default here), do the equivalent pass manually: read the markup/CSS and check it against the parent skill's General rules and Absolute bans (gradient text, side-stripe borders, eyebrow-on-every-section, ghost cards, over-rounding, cream-bg default, contrast failures, etc.). Browser/manual review below still applies.
...
4. If mutation is available and impeccable's CLI is installed, its live server can inject the in-browser detector overlay (`detect.js`) for visual evidence. Without the CLI (the default here), inspect the rendered page directly: screenshot it, check contrast and spacing against the rules, and note issues from what you observe.
...
2. **Write the snapshot** to `.impeccable/critique/<slug>-<YYYYMMDD-HHMMSS>.md` with a small YAML frontmatter (`target`, `total_score`, `p0_count`, `p1_count`, `date`) followed by the report body. This is a plain file write — no helper needed. (impeccable's CLI automates this if installed.)

3. **Read the trend** for context: list the existing `.impeccable/critique/<slug>-*.md` files (newest 5) and read their frontmatter to see how the score and P0/P1 counts have moved across runs.
