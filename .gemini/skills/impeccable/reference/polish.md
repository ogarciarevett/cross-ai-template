...
4. **Pull in any prior critique** (optional signal): If `/impeccable critique` has been run on the same target, its priority issues are a useful prior for what to address first. Read any saved critique for this target under `.impeccable/critique/` (the file `critique` writes); fold its P0/P1 items into your polish list and mention which snapshot you read. If none exists, continue without it. The critique is one input among many. Do your own pass either way.

   > Optional accelerant: impeccable's CLI (`npx impeccable`, not bundled with this dependency-free skill) can resolve and fetch the latest critique snapshot for you. Without it, just read the saved markdown directly.
